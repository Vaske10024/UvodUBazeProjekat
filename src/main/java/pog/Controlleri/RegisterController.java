...
public class RegisterController {
    ...
    public RegisterController(NoviPocetakApp app) {
        ...
        submitButton.setOnAction(e -> {
            ...
            if (!EMAIL_PATTERN.matcher(email).matches()) {
                Alert alert = new Alert(Alert.AlertType.ERROR, "Email adresa nije u ispravnom formatu (npr. primer@primer.com).");
                alert.showAndWait();
                return;
            }
            ...
            try (Connection conn = DatabaseUtil.getConnection()) {
                ...
                cstmt.setObject(10, typeChoice.getValue().equals("kandidat") ? centarMap.get(centarCombo.getValue()) : null, Types.INTEGER);
                cstmt.execute();
    ...
    private void loadFaculties() {
        try (Connection conn = DatabaseUtil.getConnection()) {
            CallableStatement cstmt = conn.prepareCall("{CALL GetFacultiesByOblast(?)}");
            cstmt.setString(1, "humanisticke nauke");
    ...
    private void loadTrainingCenters() {
        try (Connection conn = DatabaseUtil.getConnection()) {
            CallableStatement cstmt = conn.prepareCall("{CALL GetAllTrainingCenters()}");
            ResultSet rs = cstmt.executeQuery();
            while (rs.next()) {
                String naziv = rs.getString("naziv");
                int id = rs.getInt("id");
